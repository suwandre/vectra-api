...
pub async fn create_user(
    ...
    let user = sqlx::query_as!(
        User,
        r#"
        INSERT INTO users (id, wallet_address, username, xp_points, level, portfolio_value, cash_balance, created_at, updated_at)
        ...
        0i64, // Starting XP
        1i32, // Starting level
        10000.0, // Starting portfolio value
        10000.0, // Starting cash balance
        now,
        now
    )
    .fetch_one(pool)
...
/// Used for retrieving user information in authenticated requests.
...
pub async fn update_user_xp(
    ...
    let level = calculate_level_from_xp(xp_points);
    let now = Utc::now();
...
/// Simple formula: level = (XP / 1000) + 1, capped at reasonable maximum.
fn calculate_level_from_xp(xp_points: i64) -> i32 {
    let level = (xp_points / 1000) + 1;
    std::cmp::min(level as i32, 100) // Cap at level 100
}
